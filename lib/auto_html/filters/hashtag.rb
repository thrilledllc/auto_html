# Hashtag filter for twitter, facebook and instagram
AutoHtml.add_filter(:hashtag).with(:source => :twitter) do |text, options|
  ...
  elsif options[:source] == :instagram
    text.gsub(/#([^\s]+)/, '<a href="https://instagram.com/explore/tags/\1" class="hashtag" target="_blank">#\1</a>')
  end
end
